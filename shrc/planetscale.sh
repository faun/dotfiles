...
planetscale_branch_create() {
  pscale branch switch --org "$(planetscale_organization)" --database "$(planetscale_database)" --create "$(planetscale_branch)"
}
...
planetscale_branch_deploy_request_wait_for_ready() {
  ...
  while [[ "$(planetscale_branch_deploy_request_status | jq '.ready')" != "true" ]]; do echo -n "."; sleep 1; done; echo; echo "Deploy request is ready"
}
